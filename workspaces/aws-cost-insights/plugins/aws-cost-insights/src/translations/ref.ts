import { createTranslationRef } from '@backstage/core-plugin-api/alpha';

export const costInsightsMessages = {
  globalPage: {
    title: 'AWS Cost Insights',
    subtitle: 'AWS cloud infrastructure and platform cluster workload financial overview',
    totalCostTab: 'Total cost',
    breakdownTab: 'Breakdown by service',
    totalPeriodLabel: 'Total Spend ({{period}})',
    dailyAverageLabel: 'Daily Average',
    dailyAvgFormat: '~${{avg}}/day avg',
    totalCostLabel: 'AWS Total Cost',
    dateAxisLabel: 'Date',
    dailyCostTooltip: 'Daily Cost',
    noData: 'No cost data available for the selected period.',
    noGroups: 'You are not assigned to any catalog group with AWS cost tags.',
    teamGroupLabel: 'Team / Group',
    accountLabel: 'AWS Account',
    allAccounts: 'Group view (tags)',
    orgAccounts: 'Organization (all accounts)',
    fetchError: 'Error fetching cost data: {{error}}',
    fetchErrorFallback: 'Error fetching cost data',
    fetchGroupsFallback: 'Error fetching user groups',
  },
  entityCard: {
    tcoTitle: 'Consolidated Monthly TCO (AWS Cloud + Kubernetes)',
    tcoCloudOnlyTitle: 'Monthly Cloud Cost (AWS Cloud)',
    tcoUnavailableTitle: 'Consolidated Cost Unavailable',
    tcoFormat: '${{total}}/mo',
    tcoBreakdown: '(${{aws}} AWS Cloud + ${{k8s}} Kubernetes Workload)',
    tcoDisclaimer:
      'Assumes AWS-billed and in-cluster costs are disjoint; cluster nodes billed under the same AWS tags would be counted twice.',
    tcoCloudOnly: '(${{aws}} AWS Cloud · No in-cluster workload)',
    tcoCloudOnlyUnavailable: '(${{aws}} AWS Cloud · Kubernetes cost data unavailable)',
    tcoCalculating: 'Calculating consolidated costs...',
    tcoCalculatingK8s: 'Calculating Kubernetes allocation...',
    awsUnavailable: 'AWS cloud cost data unavailable',
    k8sUnavailable: 'Kubernetes cost data unavailable',
    noK8sWorkload: 'No in-cluster workload',
    perMonth: '/mo',
    defaultTagFallback: 'default',
    cloudTitle: 'Dedicated Cloud Resources (AWS Cost Explorer)',
    noAnnotationInfo:
      'Cost tracking is not configured for this entity. Add the aws.amazon.com/cost-insights-tags annotation to its catalog-info to enable AWS cost visibility.',
    cloudSubtitleSingle: 'Dedicated billed resources: {{service}}',
    cloudSubtitleMultiple: 'Cloud resources filtered by AWS tags ({{tags}})',
    totalCostPeriodLabel: 'Total in period: ${{total}}',
    totalCostTab: 'Total cost',
    breakdownTab: 'Breakdown by service',
    dailyAvgFormat: '~${{avg}}/day avg',
    noData: 'No cost data found for this entity in the selected period.',
    fetchError: 'Error fetching entity cost data: {{error}}',
    fetchErrorFallback: 'Error fetching entity cost data',
  },
  clusterCard: {
    title: 'In-Cluster Workload Spend (OpenCost / Kubernetes)',
    subtitle: 'Real-time CPU and memory consumption allocated in the platform cluster',
    badge: 'Real-time',
    workloadHeader: 'Workload / Namespace',
    cpuCostHeader: 'CPU Cost (Day)',
    ramCostHeader: 'RAM Cost (Day)',
    dailyTotalHeader: 'Daily Total',
    monthlyProjectionHeader: 'Monthly Projection',
    efficiencyHeader: 'Efficiency',
    noWorkload: 'No active workload found in the cluster for this identifier.',
    errorFetching: 'Unable to load Kubernetes cluster costs: {{error}}',
    fetchErrorFallback: 'Error fetching OpenCost data',
  },
  globalCluster: {
    title: 'Platform Cluster Workload Spend (OpenCost / Kubernetes)',
    subtitle: 'Cost allocation across all active namespaces and platform stacks in the cluster',
    badge: 'Platform Cluster',
    platformCoreBadge: 'Platform / Core',
    allNamespaces: 'All Namespaces',
    searchPlaceholder: 'Filter namespace...',
    totalClusterDaily: 'Daily In-Cluster Spend',
    totalClusterMonthly: 'Monthly In-Cluster Projection',
    efficiencyAvg: 'Cluster Efficiency Avg',
    perDay: '/day',
    perMonth: '/mo',
    errorFetching: 'Unable to load platform cluster costs: {{error}}',
    fetchErrorFallback: 'Error fetching OpenCost data',
  },
  periodSelect: {
    past30Days: 'Past 30 Days',
    past60Days: 'Past 60 Days',
    past90Days: 'Past 90 Days',
    past180Days: 'Past 6 Months (180 Days)',
    customRange: 'Custom Range...',
    customTitle: 'Select Custom Date Range',
    startDate: 'Start Date',
    endDate: 'End Date',
    apply: 'Apply',
    cancel: 'Cancel',
    presetLabel: 'Presets',
    daysSuffix: 'd',
    invalidRangeAlert: 'End date must be greater than or equal to start date.',
  },
};

export const costInsightsTranslationRef = createTranslationRef({
  id: 'plugin.aws-cost-insights',
  messages: costInsightsMessages,
});
