...
A decision recorded in a workspace's `DECISIONS.md`, numbered `PDR-NNN`, binding only that family. A decision that binds every workspace is an ADR in `docs/adr/`, cited as `plugins ADR-NNNN`.
